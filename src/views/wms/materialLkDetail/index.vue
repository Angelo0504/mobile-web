<template>
  <div class="app-container">
    <el-row :gutter="10" class="mb5">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:materialLkDetail:add']"
          >新增</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:materialLkDetail:edit']"
          >修改</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:materialLkDetail:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:materialLkDetail:export']"
          >导出</el-button
        >
      </el-col>
      <el-form
    ...
    >
      ...
      <el-table-column label="数量" align="center" prop="lkQty" />
    ...
    >
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-row>
          ...
          <el-col :span="12"
            ><el-form-item label="仓位名称" prop="positionName">
              <el-select
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="炉批号" prop="batchNo">
              <el-input v-model="form.batchNo" placeholder="请输入炉批号" />
            </el-form-item>
          </el-col>
          <el-col :span="12"
            ><el-form-item label="数量" prop="lkQty">
              <el-input v-model="form.lkQty" placeholder="请输入数量" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
...
<script>
import {
  listMaterialLkDetail,
  getMaterialLkDetail,
  delMaterialLkDetail,
  addMaterialLkDetail,
  updateMaterialLkDetail,
} from "@/api/wms/materialLkDetail";
import { listMaterial } from "@/api/wms/material";
import { listPosition } from "@/api/wms/position";

export default {
  name: "MaterialLkDetail",
  data() {
    return {
      restaurants: [],
      // 表格高度
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listMaterialLkDetail(this.queryParams).then((response) => {
        this.materialLkDetailList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getMaterialLkDetail(id).then((response) => {
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.id != null) {
            updateMaterialLkDetail(this.form).then((response) => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addMaterialLkDetail(this.form).then((response) => {
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm("是否确认删除记录？")
        .then(function () {
          return delMaterialLkDetail(ids);
        })
        .then(() => {
          this.getList();
    ...
    handleExport() {
      this.download(
        "system/materialLkDetail/export",
        {
          ...this.queryParams,
        },
        `materialLkDetail_${new Date().getTime()}.xlsx`
      );
    },
